...
p0=NULL;#p-value of WMW test
p1=NULL;
p2=NULL;#p-value of WMW-A test
for (k in 1:length(x)){
  expr_case=x[[k]];expr_control=y[[k]];
  p0[[k]]=w(expr_case,expr_control,n_case,n_control);
  for (j in 1:length(n_auxi)){
    ...
    p1[[j]]=WMWA(data,expr_case,expr_control,expr_auxi,ind,nperm)
  }
  p2[[k]]=p1
}

save(p0,p2,file='result1.Rdata')
